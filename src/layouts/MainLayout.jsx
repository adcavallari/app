...
export default function MainLayout() {
  ...
  // =========================================================================
  // Exemplo de roles do banco: 'admin', 'pastor', 'lider', 'membro'
  const userRole = profile?.role || 'membro'; 
  // Exemplo de ministérios: ['louvor', 'midia', 'obreiros']
  ...
  const allPages = [
    ...
    { id: 'announcements', path: '/announcements', name: 'Avisos (Admin)', icon: Megaphone, roles: ['admin', 'pastor', 'lider'] },
  ...
  const filteredNavigation = allPages.filter(page => {
    // Se não tiver restrição de role, ou se a role do user estiver na lista de roles permitidas
    const hasRoleAccess = page.roles ? page.roles.includes(userRole) : true;
    
    // Se a página for específica para um ministério (ex: Louvor), verifica se o user pertence a ele. (Admin e Pastor veem sempre)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#09090b] flex text-slate-900 dark:text-slate-50 font-sans">
      
      {/* =========================================================
          SIDEBAR (Visível apenas em Desktop)
          ========================================================= */}
      <aside className="hidden md:flex w-72 flex-col fixed h-screen p-4 z-20">
        <div className="flex-1 flex flex-col bg-white/60 dark:bg-zinc-900/40 backdrop-blur-2xl border border-slate-200/50 dark:border-white/5 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-6 flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight text-slate-900 dark:text-white">Hub Cavallari</h1>
              <BadgeRole role={userRole} />
            </div>
          </div>

          <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto scroll-smooth [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-200 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full">
              ...
              return (
                <button
                  ...
                  className={cn("w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group",
                    isActive ? "bg-indigo-600 text-white shadow-md shadow-indigo-500/20" : "text-slate-600 hover:bg-slate-100/80 dark:text-zinc-400 dark:hover:bg-white/5"
                  )}
                >
          ...
          <div className="p-4 mt-auto border-t border-slate-200/50 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
            <div className="flex items-center gap-3 p-2 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-white/5 transition-colors" onClick={() => navigate('/profile')}>
              ...
              <div className="flex-1 min-w-0 text-left">
                <p className="text-sm font-bold truncate text-slate-900 dark:text-white">{profile?.full_name || 'Utilizador'}</p>
                <p className="text-xs text-slate-500 dark:text-zinc-400 truncate capitalize">{userRole}</p>
              </div>
            </div>
            <div className="flex gap-2 mt-3 px-2">
               <button onClick={toggleTheme} className="flex-1 flex justify-center p-2 rounded-lg bg-white dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 hover:text-indigo-600 shadow-sm border border-slate-200 dark:border-white/5 transition-colors">
      ...
      <main className="flex-1 md:pl-[19rem] flex flex-col min-h-screen relative">
        ...
        <header className="md:hidden flex items-center justify-between p-4 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-white/5 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg tracking-tight">Hub Cavallari</span>
          </div>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-white text-xs" onClick={() => navigate('/profile')}>
            {profile?.full_name?.charAt(0) || 'U'}
          </div>
        </header>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-40 flex flex-col justify-end">
            ...
            <div className="relative bg-white dark:bg-[#121214] w-full rounded-t-3xl shadow-2xl pb-28 animate-in slide-in-from-bottom-full duration-300 ease-out border-t border-slate-200 dark:border-white/10">
              
              <div className="w-12 h-1.5 bg-slate-200 dark:bg-zinc-800 rounded-full mx-auto mt-4 mb-6"></div>
              ...
              <div className="grid grid-cols-4 gap-y-6 gap-x-2 px-4 max-h-[50vh] overflow-y-auto">
                {filteredNavigation.map((item) => (
                  <button 
                    key={item.id} 
                  ...
                  >
                    <div className={cn(
                      ...
                      location.pathname.includes(item.path) 
                        ...
                        : "bg-slate-50 border-slate-200 dark:bg-zinc-900 dark:border-white/5 group-hover:border-indigo-500/30"
                    )}>
                      <item.icon className={cn("w-6 h-6", location.pathname.includes(item.path) ? "text-white" : "text-slate-600 dark:text-zinc-400")} />
                    </div>
                    <span className="text-[11px] font-semibold text-center leading-tight text-slate-600 dark:text-zinc-400 max-w-[70px] truncate">
                      {item.name}
                    </span>
                  </button>
                ))}
        ...
        {/* =========================================================
            ...
            ========================================================= */}
        <nav className="md:hidden fixed bottom-6 left-4 right-4 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-slate-200/80 dark:border-white/10 rounded-3xl shadow-2xl z-50 px-2 py-2">
              ...
              if (item.isToggle) {
                ...
                return (
                  <button 
                    ...
                    className="relative flex flex-col items-center justify-center w-14 h-14 -mt-6 bg-indigo-600 text-white rounded-full shadow-lg shadow-indigo-500/40 border-4 border-slate-50 dark:border-[#09090b] active:scale-95 transition-transform"
                  >
              ...
              return (
                <button 
                  ...
                  className={cn(
                    ...
                    isActive ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200"
                  )}
                >
...
function BadgeRole({ role }) {
  const rolesMap = {
    ...
    membro: { label: 'Membro', color: 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-white/5 dark:text-zinc-400 dark:border-white/5' },
  };

...
export default function Auth() {
  ...
  useEffect(() => {
    // Carrega a preferência ao abrir a página
    const isRemembered = localStorage.getItem('@Worship:rememberMe') === 'true';
    if (isRemembered) {
      setRememberMe(true);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // 1. ANTES DE LOGAR: Definimos a regra no navegador!
      // O Supabase (configurado no passo 2) vai ler esta variável para decidir onde salvar a sessão.
      if (rememberMe) {
      ...
      // 2. AGORA logamos. O Supabase já saberá se deve manter a sessão ao fechar o navegador.
  ...
  return (
    <div className="min-h-screen flex w-full bg-slate-50 dark:bg-[#09090b] relative">
      ...
      <div className="hidden lg:flex relative w-1/2 items-center justify-center overflow-hidden bg-zinc-900 border-r border-slate-200 dark:border-white/5 shadow-2xl z-10">
        ...
        <div className="relative z-10 w-full max-w-lg p-12 text-white">
          <div className="flex items-center gap-4 mb-12">
            <img src={Logo} alt="AD Cavallari" className="w-16 h-16 object-contain"/>
          ...
          {!isRegistering ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <form className="space-y-5" onSubmit={handleLogin}>
                <Input label="E-mail" type="email" placeholder="seu@email.com" icon={User} value={email} onChange={(e) => setEmail(e.target.value)} required />
                <Input label="Palavra-passe" type="password" placeholder="••••••••" icon={Settings} value={password} onChange={(e) => setPassword(e.target.value)} required />
            ...
            <div className="animate-in fade-in slide-in-from-right-8 duration-500">
              ...
              <form className="space-y-5" onSubmit={handleRegister}>
                <Input label="Nome Completo" type="text" placeholder="João da Silva" icon={User} value={fullName} onChange={(e) => setFullName(e.target.value)} required />
                <Input label="E-mail" type="email" placeholder="seu@email.com" icon={Mail} value={email} onChange={(e) => setEmail(e.target.value)} required />
                <div className="space-y-1">
                  <Input label="Palavra-passe" type="password" placeholder="••••••••" icon={Settings} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
